(function($) {
	$.fn.circleMagic = function(options) {
		var width,
			...
			animateHeader = true;
		var circles = [];
		var settings = $.extend({ color: "rgba(255,255,255,.5)", radius: 10, density: 0.3, clearOffset: 0.2 }, options);
		...
		function Circle() {
			...
			this.draw = function() {
				...
				ctx.beginPath();
				ctx.rect(that.pos.x, that.pos.y, that.scale * settings.radius, that.scale * settings.radius, false);
